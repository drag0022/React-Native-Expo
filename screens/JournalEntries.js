				...
				renderItem={(item) => (
					<View>
						...
						<Card containerStyle={{
							...
							borderRadius:10}}
						>
							<Card.Title style={{color: '#A10582'}}>Title: {item.item.title ? item.item.title : 'no title available'}</Card.Title>				
							<Card.Divider/>	
							{item.item.image ? (
								<Card.Image		
                  resizeMode="cover"
							...
							<Text style={styles.text}>ID: {item.item.id ? item.item.id : 'no id available'}</Text>
						...
						{item.item.location ? (
							...
							{item.item.location ? (
								...
								<Text style={styles.text}>no location information available</Text>
							)}
						</Card>
					</View>
				)}
